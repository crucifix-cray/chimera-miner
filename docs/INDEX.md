...
**Updated:** 2026-09-21 ~18:00 UTC

## Where we are now
- **1 miner LIVE:** cell-16 / session-2 (`altonlehman16@gmail.com`) / project `7d6f77a6` / **`daemon.py --mode full`** / Chromium. Health checks every ~3 min; smart revive on shell death.
...
├── Health (~180s / ~45s on fail):
│   ├── shell_worker_status (proxy-404 / nodoc / probe=0 → dead)
│   ├── revive: goto chat → re-login if needed → lovable → inject
│   └── 3 revive fails → restart browser (outer loop)
└── Token refresh 40m (page_lock vs revive)
```
