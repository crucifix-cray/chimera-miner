...
def resolve_proxy() -> dict | None:
    ...
    import socket
    # Force no proxy for local headed runs
    if os.environ.get("CHIMERA_NO_PROXY") == "1":
        print("⚠️  CHIMERA_NO_PROXY=1: skipping proxy detection", file=sys.stderr)
        return None
...
async def relogin_session(browser, config: dict, session_id: str, existing_context=None) -> str:
    ...
    # Reuse existing context if provided, otherwise create new
    if existing_context:
        context = existing_context
        page = await context.new_page()
        close_context_after = False
    else:
    ...
    try:
        async with _launch_browser() as browser:
            ...
            if not await check_session_valid(chat_page):
                ...
                relogin_done = True
                if result == "ok":
                    print("✅ Re-login OK - continuing with same context")
                    # Cookies already in context, just go back to chat
                    await goto_retry(chat_page, chat_url)
